...
// ListStacks returns all Stacks created under the current Project or all stacks under the scope of the current Pulumi Access Token (PAT).
// This queries underlying backend and may return stacks not present in the Workspace (as Pulumi.<stack>.yaml files).
func (l *LocalWorkspace) listStacks(ctx context.Context, listAll bool) ([]StackSummary, error) {
	var stacks []StackSummary
	var args []string = []string{"stack", "ls", "--json"}
	if listAll {
